...
- 完整交易历史仍未实现
- WebSocket 推送未接入主流程
- 若发送交易返回 `-32010`，表示节点默认关闭 `eth_sendRawTransaction`；
  请在开发环境使用 `zerochain run --rpc-enable-eth-write-rpcs` 启动节点
...
- 创建方式：随机 `ed25519` 私钥
- 导入方式：原生私钥
- 地址规则：`ZER0x` + `keccak256(public_key_32)` 后 20 字节（按 ETH 风格 checksum 大小写编码）
